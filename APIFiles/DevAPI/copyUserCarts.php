...
$token = $headers['Token'];
if ($getuserFrom == "" || $getuserTo == "" || $getpostid == "" || $uType == "" || $token == ""){
    $response = "{\"status\": \"error\"}";
    die($response);
}
else 
{
	if($verify=verify_jwt_by_userid($token,$getuserTo)){
    }else{
...
$q = "call copyUserCarts(\"$getuserFrom\", \"$getuserTo\", \"$getpostid\",\"$uType\");";
$json_arry = array();
$json_arry1 = array();
