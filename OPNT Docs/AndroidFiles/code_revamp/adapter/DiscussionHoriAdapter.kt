...
class DiscussionHoriAdapter(
    val context: Context,
    private val listener: OnItemClickListener
) : RecyclerView.Adapter<DiscussionHoriAdapter.ItemViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ItemViewHolder, position: Int) {
        ...
        holder.apply {
            ...
            if (selectedTopic == topicList[pos].TOPICID) {
                ...
                binding.customTabIv.setColorFilter(getColor(context, R.color.colorPrimary))
                binding.customTabTv.setTextColor(getColor(context, R.color.colorPrimary))
                ...
                binding.customTabIv.setColorFilter(getColor(context, R.color.colourGrey))
                binding.customTabTv.setTextColor(getColor(context, R.color.colourGrey))
    ...
    fun updateList(topicsList: ArrayList<GetUserTopicResponseItem>) {
    ...
    private fun updateHeader(){
       val value =  topicList.find {
            it.TOPICID == selectedTopic
        }
        if(value==null && topicList.isNotEmpty()){
            selectedTopic = topicList[0].TOPICID?:-1
            Preference(context).saveIntPref(Constants.TOPICCARTID, selectedTopic)
        }
    }
