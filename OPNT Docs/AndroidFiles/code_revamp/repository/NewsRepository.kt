...
class NewsRepository @Inject constructor(private val userAPI: UserAPI) {
    ...
    private val _getInstreamNwResponseLiveData =
        MutableLiveData<NetworkResult<GetInstreamNwResponse>>()
    val getInstreamNwResponseLiveData: LiveData<NetworkResult<GetInstreamNwResponse>>
        get() = _getInstreamNwResponseLiveData

    suspend fun getInstreamNW(getInstreamNwRequest: GetInstreamNwRequest) {
        try {
            _getInstreamNwResponseLiveData.postValue(NetworkResult.Loading())
            val response = userAPI.getInstreamNw(token, xKey, getInstreamNwRequest)
            handleGetINstreamNwResponse(response)
        } catch (e: Exception) {
            e.printStackTrace()
        }
    }

    private fun handleGetINstreamNwResponse(response: Response<GetInstreamNwResponse>) {
        if (response.isSuccessful && response.body() != null) {
            _getInstreamNwResponseLiveData.postValue(NetworkResult.Success(response.body()!!))
        } else if (response.errorBody() != null) {
            val errorObj = response.errorBody()!!.charStream().readText()
            _getInstreamNwResponseLiveData.postValue(NetworkResult.Error(errorObj))
        } else {
            _getInstreamNwResponseLiveData.postValue(NetworkResult.Error("Something Went Wrong"))
        }
    }
    ...
    suspend fun getInstreamNWAnti(getInstreamNwRequest: GetInstreamNwRequest) {
        try {
            _getInstreamNwResponseLiveData.postValue(NetworkResult.Loading())
            val response = userAPI.getInstreamAnti(token, xKey, getInstreamNwRequest)
            handleGetINstreamNwResponse(response)
        } catch (e: Exception) {
            e.printStackTrace()
        }
