...
SELECT SCRAPE_TOPIC, COUNTRY_CODE, COUNT(1)  FROM WEB_SCRAPE_RAW_L where SCRAPE_DATE >= CURRENT_DATE() - INTERVAL 2 DAY AND IFNULL(MOVED_TO_POST_FLAG, 'N') = 'N'
...
SELECT COUNT(1) FROM WEB_SCRAPE_RAW_L WHERE SCRAPE_DATE > CURRENT_DATE() - INTERVAL 2 DAY ;
DELETE FROM WEB_SCRAPE_RAW_L WHERE SCRAPE_DATE <= CURRENT_DATE() - INTERVAL 2 DAY ;
