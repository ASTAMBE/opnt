...
-- IT IS CRITICAL TO KEEP THE OPN_USER_INTERESTS IN SYNC WITH THE CARTS AFTER EVERY DELETE OPERATION ON THE CARTS
...
-- Find the ROW_IDs from OPN_USER_CARTS that are: not the USERIDs who created the STD words, and not the NEWS_ONLY rows (cart rows that contain NEWS_ONLY kws) and those created < 15 days ago
...
-- KEEPING THE OPN_USER_INTERESTS IN SYNC
...
;
-- LETS FIND OUT HOW MANY KWS IN BOT CARTS THAT ARE OLDER THAN 30 DAYS VS NEWER

SELECT C.USERID, SUM(CASE WHEN C.CREATION_DTM < CURRENT_DATE() - INTERVAL 30 DAY THEN 1 ELSE 0 END) OLDER
, SUM(CASE WHEN C.CREATION_DTM >= CURRENT_DATE() - INTERVAL 30 DAY THEN 1 ELSE 0 END) NEWER
FROM OPN_USER_CARTS C, OPN_USERLIST U WHERE C.USERID = U.USERID AND U.BOT_FLAG = 'Y' AND C.TOPICID = 1 AND C.KEYID > 160000
GROUP BY C.USERID ORDER BY 2 DESC ;

-- THEN FIND OUT WHAT KIND OF KWS ARE THERE IN ONE OF THESE USERS:

SELECT C.USERID, C.KEYID, C.CART, C.CREATION_DTM, K.KEYWORDS, K.COUNTRY_CODE FROM OPN_USER_CARTS C, OPN_P_KW K WHERE C.KEYID = K.KEYID AND C.TOPICID = 1 AND C.USERID = 1021452 
AND C.CREATION_DTM < CURRENT_DATE() - INTERVAL 30 DAY AND C.KEYID > 106000;

-- BLANKET DELETE ALL THE BOT CART ROWS THAT ARE > 30 DAYS OLD BUT KEYID > 106000 

DELETE FROM OPN_USER_CARTS C WHERE C.CREATION_DTM < CURRENT_DATE() - INTERVAL 30 DAY AND C.KEYID > 106000
AND C.USERID IN (SELECT USERID FROM OPN_USERLIST WHERE IFNULL(BOT_FLAG, 'N') = 'Y') ;
...
SELECT COUNT(1) FROM OPN_USER_CARTS where KEYID < 106000 ;
...
SELECT * FROM OPN_USER_INTERESTS WHERE USERID = 1000001 ;
