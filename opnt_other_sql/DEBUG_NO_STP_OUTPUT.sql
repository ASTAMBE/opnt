...
/* If the above query doesn't return much for any topicid/ccode combo then that combo is having STP issues */

-- Step 1.1: Since the DISCCNT is non-zero, we have addressed the Discussions creation problem at least.
-- Now why the POSTCNT is still zero
SELECT * FROM WEB_SCRAPE_RAW ORDER BY NEWS_DATE DESC ;
...
-- Check if WSR has the three main issues resolved: MOVED_TO_POST_FLAG, TAG_DONE_FLAG  and the NEWS_DATE

SELECT STR_TO_DATE(NEWS_DATE, '%Y-%m-%d') LND, COUNT(1) FROM WEB_SCRAPE_RAW GROUP BY STR_TO_DATE(NEWS_DATE, '%Y-%m-%d') ORDER BY 1 DESC ;

-- Now check if the scrapes in WSR are deduped already

SELECT COUNT(1) FROM WEB_SCRAPE_DEDUPE ; -- 11137

-- WTF? the WEB_SCRAPE_DEDUPE should be empty unless it is in the midst of deduping. Why is it still full ?
...
SELECT C.TOPICID, K.KEYID, K.KEYWORDS, K.SCRAPE_TAG1, K.SCRAPE_TAG2, SUM(CASE WHEN U.COUNTRY_CODE = 'USA' THEN 1 ELSE 0 END) USACOUNT
...
AND K.KEYWORDS LIKE '%NEWS' AND K.NEWS_ONLY_FLAG = 'Y' GROUP BY C.TOPICID, K.KEYID, K.KEYWORDS, K.SCRAPE_TAG1, K.SCRAPE_TAG2  ORDER BY K.KEYID ;
...
SELECT BASE.USERID, UL.USER_UUID, BASE.TOPICID, T.INTEREST_NAME, T.INTEREST_CODE, NOW(), BASE.SELECTED_KW_COUNT, UL.USERNAME
FROM
(SELECT C.USERID, C.TOPICID, COUNT(C.KEYID) SELECTED_KW_COUNT FROM OPN_USER_CARTS C GROUP BY C.USERID, C.TOPICID) BASE,
(SELECT U.USERID, U.USER_UUID, U.USERNAME FROM OPN_USERLIST U) UL,
(SELECT TOPICID, TOPIC INTEREST_NAME, CODE INTEREST_CODE FROM OPN_TOPICS) T
WHERE BASE.USERID = UL.USERID AND BASE.TOPICID = T.TOPICID 
 ;
