...
/* DELETE ROWS FROM THE CART TABLE THAT ARE ASSIGNED TO WRONG CCODE USERS  */

DELETE FROM OPN_USER_CARTS WHERE ROW_ID IN (SELECT ROW_ID FROM (
SELECT C.ROW_ID, C.USERID, C.TOPICID, C.KEYID, C.CREATION_DTM, K.KEYWORDS, K.KW_URL, K.COUNTRY_CODE KW_COUNTRY_CODE, U.COUNTRY_CODE UCON, U.TRUE_COUNTRY_CODE,
P.POST_ID, P.POST_URL, P.URL_TITLE, P.STP_PROC_NAME
FROM OPN_USER_CARTS C, OPN_P_KW K, OPN_USERLIST U, OPN_POSTS P
WHERE C.KEYID = K.KEYID AND C.KEYID = P.KEYID AND C.USERID = U.USERID AND IFNULL(K.NEWS_ONLY_FLAG, 'N') <> 'Y'
AND K.COUNTRY_CODE = 'GGG' AND U.COUNTRY_CODE <> 'GGG' AND P.STP_PROC_NAME = 'TCC_DISCUSSION')Q)
 ; 
