...
SELECT ROUTINE_TYPE, ROUTINE_NAME FROM INFORMATION_SCHEMA.ROUTINES WHERE ROUTINE_SCHEMA = 'opntprodd' AND ROUTINE_DEFINITION LIKE '%OPN_USER_INTEREST%' ;

SELECT ROUTINE_TYPE, ROUTINE_NAME, LAST_ALTERED, LENGTH(ROUTINE_DEFINITION) RLEN FROM INFORMATION_SCHEMA.ROUTINES 
WHERE ROUTINE_SCHEMA = 'opntprodd' and LAST_ALTERED > '2024-03-26 07:12:27' ORDER BY 2  ;
...
SELECT cast(CREATION_DATE as date),  COUNT(1) FROM OPN_USERLIST WHERE USER_TYPE in ('USER', 'GUEST') AND CREATION_DATE > CURRENT_DATE - INTERVAL 10 DAY GROUP BY cast(CREATION_DATE as date) ORDER BY 1;
