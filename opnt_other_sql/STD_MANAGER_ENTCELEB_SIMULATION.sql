...
SELECT COUNT(1) FROM WEB_SCRAPE_RAW_L where NEWS_DATE >= CURRENT_DATE() - INTERVAL 1 DAY ;

SELECT CAST(NEWS_DATE AS DATE) ND, COUNTRY_CODE, COUNT(1)  FROM WEB_SCRAPE_RAW_L where SCRAPE_DATE >= CURRENT_DATE() - INTERVAL 30 DAY 
AND IFNULL(MOVED_TO_POST_FLAG, 'N') = 'N' GROUP BY CAST(NEWS_DATE AS DATE), COUNTRY_CODE ORDER BY 1, 2 DESC ;

DELETE FROM WEB_SCRAPE_RAW_L where CAST(NEWS_DATE AS DATE)  < '2024-12-10' ;
...
SELECT COUNT(1) FROM WEB_SCRAPE_RAW_L where SCRAPE_TOPIC  IN ('CELEB', 'ENT', 'SCIENCE') AND NEWS_DATE <= CURRENT_DATE() - INTERVAL 1 DAY ;
