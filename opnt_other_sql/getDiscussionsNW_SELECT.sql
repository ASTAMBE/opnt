...
FROM
    ...
    FROM
        OPN_USER_CARTS C1
    WHERE
        C1.USERID = 1026625 AND C1.TOPICID = tid) A, (SELECT 
        C2.USERID,
            CU.BOT_FLAG,
            C2.TOPICID,
            C2.CART,
    ...
    WHERE
        C2.USERID = CU.USERID
        AND C2.TOPICID = 10 -- AND CU.BOT_FLAG <> 'Y'
            AND C2.USERID NOT IN (SELECT 
                OUUA.ON_USERID
            FROM
                OPN_USER_USER_ACTION OUUA
            WHERE
                OUUA.BY_USERID = 1026625
                    AND OUUA.TOPICID = 10
    ...
    (SELECT 
        CAUSE_POST_ID,
            ...
            END) HCOUNT
    FROM
        OPN_USER_POST_ACTION  WHERE TOPICID = 10
    ...
    OPN_USER_POST_ACTION UP ON INSTREAM.POST_ID = UP.CAUSE_POST_ID
        AND UP.ACTION_BY_USERID = 1026625
        LEFT OUTER JOIN
    (SELECT 
        BK.POST_ID
    FROM
