...
BEGIN
 ...
 earlier it was URL1 =  UPPER(substring_index(substring_index(substring_index(EMB, '://',-1), '/',1), '.',-2)) 
  ...
  08/14/2023 AST: The proc was using OPN_CLEAN_DOMAINS.U_DOMAIN to check for clean domains. It was changed to
  SITE_NAME because that is the new type that comes form embedded content (domain.ext)
...
WHEN UCNT = 1 THEN SET URL1 =  TRIM(substring_index(EMB, ',', 1)) ;
 CASE WHEN URL1 IN (select SITE_NAME from OPN_CLEAN_DOMAINS) THEN UPDATE OPN_POST_COMMENTS SET CLEAN_COMMENT_FLAG = 'Y'
 ...
 CASE WHEN (URL1 IN (SELECT SITE_NAME FROM OPN_CLEAN_DOMAINS) AND URL2 IN (SELECT SITE_NAME FROM OPN_CLEAN_DOMAINS) ) THEN
 ...
 CASE WHEN (URL1 IN (SELECT SITE_NAME FROM OPN_CLEAN_DOMAINS) AND URL2 IN (SELECT SITE_NAME FROM OPN_CLEAN_DOMAINS)
