...
WHEN UCNT = 1 THEN SET URL1 =  TRIM(substring_index(EMB, ',', 1)) ;
 CASE WHEN URL1 IN (select SITE_NAME from OPN_CLEAN_DOMAINS) THEN UPDATE OPN_POSTS SET CLEAN_POST_FLAG = 'Y' 
 , POST_PROCESSED_FLAG = 'Y', POST_PROCESSED_DTM = NOW()   WHERE POST_ID = PID ; 
