...
(SELECT CLUSTER_ID FROM OPN_NC_CLUSTERS WHERE USERID = NEW.POST_BY_USERID AND TOPICID = NEW.TOPICID)));
    ...
    10/29/2024 AST: Adding AND U.BOT_FLAG <> 'Y' to exclude the BOTs from getting the notifs
    
    11/17/2024 AST: Restricting the notifs to only : Discussion posts and same CCODE as the postor
    AND NEW.POSTOR_COUNTRY_CODE = U.COUNTRY_CODE AND NEW.DEMO_POST_FLAG <> 'Y'
...
SET POST_EXCRPT = 'A new discussion has been started';
...
CASE WHEN NEW.TOPICID <> 3 THEN

INSERT IGNORE INTO OPN_PUSH_LAUNCH(USERID, USER_UUID, APP_TOKEN, USER_PLATFORM, PUSH_TYPE
...
, 1 , POSTTOPIC, NEW.TOPICID, POST_EXCRPT, 'New discussion started in'
...
WHERE U.IDENTIFIER_TOKEN IS NOT NULL AND U.BOT_FLAG <> 'Y'
AND U.USERID = NW.USERID 
AND NEW.POSTOR_COUNTRY_CODE = U.COUNTRY_CODE
AND NEW.DEMO_POST_FLAG <> 'Y'
;

WHEN NEW.TOPICID = 3 THEN
...
WHERE U.IDENTIFIER_TOKEN IS NOT NULL AND U.BOT_FLAG <> 'Y'
AND U.USERID = NW.USERID 
AND NEW.POSTOR_COUNTRY_CODE IN (U.COUNTRY_CODE, 'GGG')
AND NEW.DEMO_POST_FLAG <> 'Y'
;
