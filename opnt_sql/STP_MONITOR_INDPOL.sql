...
-- 
 ...
 DELIMITER //
DROP PROCEDURE IF EXISTS STP_MONITOR_INDPOL //
CREATE PROCEDURE STP_MONITOR_INDPOL(WSR_FILE VARCHAR(300))
THISPROC: BEGIN
...
VALUES 
(WSR_FILE, WFD, 'STP_BOLO_INDPOL', CDTM)
/*
...
-- Adding the STP_BOLO_INDPOL 

SET R_ID = (SELECT ROW_ID FROM OPN_STP_MONITOR WHERE WSR_FILE_NAME = WSR_FILE AND STP_MONITOR_PROCESS = 'STP_BOLO_INDPOL' AND CREATION_DTM = CDTM) ;
SET SPC = (SELECT COUNT(1) FROM OPN_POSTS WHERE CLEAN_POST_FLAG = 'Y') ;
SET SWC = (SELECT COUNT(1) FROM WEB_SCRAPE_RAW) ;
CALL STP_BOLO_INDPOL() ;
