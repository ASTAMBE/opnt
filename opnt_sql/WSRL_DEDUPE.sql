...
-- WSRL_DEDUPE
 ...
 DELIMITER //
DROP PROCEDURE IF EXISTS WSRL_DEDUPE //
CREATE PROCEDURE WSRL_DEDUPE(SCRAPESRC varchar(45))
BEGIN
...
UPDATE WEB_SCRAPE_RAW_L SET NEWS_DTM_RAW = NOW() - INTERVAL 3 DAY WHERE SCRAPE_SOURCE = SCRAPESRC
...
, SUBSTR(NEWS_HEADLINE, 1, 500), SUBSTR(NEWS_EXCERPT, 1, 300), IFNULL(MOVED_TO_POST_FLAG, 'N'), COUNTRY_CODE, SCRAPE_TAG1, SCRAPE_TAG2, SCRAPE_TAG3
FROM WEB_SCRAPE_RAW_L WHERE IFNULL(MOVED_TO_POST_FLAG, 'N') = 'N' AND SCRAPE_SOURCE = SCRAPESRC  ;
...
DELETE FROM WEB_SCRAPE_RAW_L WHERE SCRAPE_SOURCE = SCRAPESRC ;

INSERT INTO WEB_SCRAPE_RAW_L(SCRAPE_SOURCE, SCRAPE_TOPIC, SCRAPE_DATE, NEWS_DATE
...
DELETE FROM WEB_SCRAPE_RAW_L  WHERE IFNULL(NEWS_URL, 'BLANK') IN (SELECT IFNULL(NEWS_URL, 'NOURL') FROM WSR_CONVERTED WHERE SCRAPE_SOURCE = SCRAPESRC ) ;
-- DELETE FROM WSR_DEDUPE_NDTM WHERE SCRAPE_SOURCE = SCRAPESRC ;
