...
/*
	...
	11/28/2021 AST: Incorporating the APPNAME = 'DB8-IND-POL' logic
		Also added the filter to make sure that partial cart matches are not included in the ANTI network
...
SELECT     U1.USERID, U1.USERNAME INTO ORIG_UID , THIS_UNAME FROM
    OPN_USERLIST U1 WHERE     U1.USER_UUID = usid;
SELECT 
    OP1.TOPICID, OP1.POST_BY_USERID
INTO TID , CAUSE_POST_BYUID FROM
    OPN_POSTS OP1
WHERE
    OP1.POST_ID = postid;
SELECT 
    U2.USERNAME
INTO PARENT_UNAME FROM
    OPN_USERLIST U2
WHERE
    U2.USERID = CAUSE_POST_BYUID;
...
WHERE C1.USERID = orig_uid AND C1.TOPICID = topicid) C ,
