...
DROP PROCEDURE IF EXISTS `getInstreamBoloANTI`;
DELIMITER //
CREATE PROCEDURE getInstreamBoloANTI(userid varchar(45), topicid INT , fromindex INT, toindex INT)
THISPROC: BEGIN
...
VALUES(UNAME, orig_uid, userid, NOW(), 'getInstreamBoloANTI', CONCAT(topicid,'-',toindex));
...
AND A.KEYID = B.KEYID AND A.CART <> B.CART 
AND B.USERID NOT IN 
(SELECT DISTINCT D.USERID FROM 
(SELECT C1.USERID, C1.TOPICID, C1.CART, C1.KEYID FROM OPN_USER_CARTS C1 WHERE C1.USERID = orig_uid AND C1.TOPICID = topicid) C ,
...
INNER JOIN (SELECT * FROM OPN_USERLIST WHERE BOT_FLAG = 'Y'
) OU
