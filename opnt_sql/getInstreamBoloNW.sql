...
-- USE `opntprod`;
DROP procedure IF EXISTS `getInstreamBoloNW`;

DELIMITER $$
-- USE `opntprod`$$
CREATE  PROCEDURE `getInstreamBoloNW`(userid varchar(45), topicid INT , fromindex INT, toindex INT
)
thisproc: BEGIN
...
VALUES(UNAME, orig_uid, userid, NOW(), 'getInstreamBoloNW', CONCAT(topicid,'-',toindex));
...
AND A.KEYID = B.KEYID AND A.TOPICID = topicid
...
INNER JOIN (SELECT * FROM OPN_USERLIST WHERE BOT_FLAG = 'Y') OU
